...
诺文斯克大地图 + 13 张地区地图的离线浏览器：点开地图、缩放平移、随手画圈画框记任务，标注全部存在自己电脑上。Windows 免安装，**全程不联网**。

![诺文斯克大地图](docs/screenshots/01-overview.jpg)

## 下载

到 [Releases](../../releases/latest) 页面下载最新的 `尼沙皇版图浏览小工具-1.0.1-portable.exe`（约 123 MB），双击就能用。
...
- **标注**：圆、方框、箭头、自由画笔、地图文字，6 种颜色 3 档线宽 3 档字号；按地图分别保存，换屏幕分辨率也不会偏移。
- **橡皮擦**：按 `E` 切到橡皮擦，按住左键划过要删的标注就会擦掉，一次拖动算一步撤销（`Ctrl+Z` 整段恢复）。
- **箭头标字**：画完箭头会在箭头中段弹输入框，写上的字符锁在中点、跟着箭头一起挪动，始终水平好认。
- **地图文字**：按 `T` 点地图就地打字（回车落字、Esc 取消），文字带一圈深色描边，地图再花也看得清；选中后可拖、可改色、可改字号、可删除。
- **标注编辑**：切到选择工具，点中任意一条就能拖动、改颜色线宽、删除，也能整层隐藏（数据还在）；双击文字或箭头中段字符可以直接改字。
- **任务框**：给任务起个名选个颜色，在地图上拖出地点，做完点一下打勾。同一个任务可以横跨多张地图，大地图上会用任务颜色标出哪些地区挂着它。
- **赛季文件刷点**：地图左下先留了入口占位，功能还在做（点一下会提示在施工）。
- **更新公告**：每次版本更新后第一次打开会弹一次公告，可以勾「不再提醒」；右下角版本号旁边的「公告」按钮可以随时再看一遍。
...
| --- | --- |
| `V` / `S` | 移动 / 选择并拖动单条标注 |
| `C` / `R` / `A` / `P` / `T` / `E` | 圆 / 方框 / 箭头 / 自由画笔 / 地图文字 / 橡皮擦 |
| `Ctrl+Z` / `Ctrl+Y` | 撤销 / 重做 |
| `Delete` | 删除选中的标注 |
| 按住 `空格` | 临时拖动平移 |
| `+` / `-` | 放大 / 缩小 |
...
| `settings.json` | 界面偏好（面板收起状态、最近浏览、启动位置、公告是否不再提醒） |
...
npm start        # 开发模式启动
npm run smoke    # 端到端自检（129 项，用临时数据目录，不动本机数据）
npm run archive  # 把当前版本源码 + exe 归档到 archive/v<版本>/，出问题能回退
...
```powershell
node tools/verify-packaged.js "dist/尼沙皇版图浏览小工具-1.0.1-portable.exe"
```

### 版本归档与回滚

每个版本发布前先 `npm run archive`：脚本会把当前工程快照（除 `node_modules/`、`dist/`、原图目录外）写进 `archive/v<版本>/source/`，把 `dist/` 下同版本的免安装 exe **移动**进归档目录，并生成带 SHA256 的 `manifest.json` 与索引 `archive/README.md`。脚本只做复制、移动、写清单，不删除源码；同一版本重复执行会安全中止。

回滚就是把 `archive/v<版本>/source/` 覆盖回工程根目录再 `npm start`，或者直接发归档里的老 exe。标注数据向前兼容：v1.0.1 只是在形状里加了 `text` / `label` 字段，退回 v1.0.0 不会丢数据（文字会显示成一个小点、箭头中段字符不显示）。
...
| `config/announcements.json` | 启动时弹的更新公告内容（改这个文件就能换公告，不用改代码） |
...
| `docs/` | README 用的截图、视频封面，以及介绍视频的分镜脚本 |
| `archive/v<版本>/` | 每个版本的源码快照 + 免安装 exe + 带 SHA256 的清单，用来回滚 |
